...
public:
    ...
    Model(string const &path, ShaderType type, bool gamma = false) : gammaCorrection(gamma)
    {
        this->type = type;
        loadModel(path);
    }
...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        if(type == INSTANCE_TEX_SHADER)
            return Mesh(vertices, indices, textures, type, 1000);
        else
